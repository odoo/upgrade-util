# -*- coding: utf-8 -*-
"""
Utility functions to report upgrade changes.

The info generated by the reporting functions will be visible as posts
in the Administrator channel post upgrade.
"""
...
def report_message(message, category="Other", format="text"):
    """Append the upgrade report with a new entry, formatted according to `format`.

    :param str message: what to report.
    :param str category: Title of a report entry.
    :param str format: format used to render `message`. Supported values:

                      - ``text``: plain text, escaped and displayed as-is.
                      - ``html``: rendered as raw HTML markup.
                      - ``md``: rendered as Markdown and converted to HTML.
                      - ``rst``: rendered as reStructuredText and converted to HTML.
    """
...
def get_anchor_link_to_record(model, id, name=None, action_id=None):
    """Get an HTML link to a record in Odoo UI.

    :param str model: technical model name.
    :param int id: record ID to link to.
    :param str name: link text (optional); defaults to "model(id=id)" if omitted.
    :param int action_id: optional action to open the record through.
    :return: HTML anchor tag, wrapped in Markup if available.
    :rtype: str or :class:`markupsafe.Markup`
    """
