"""
Helpers to manipulate views/template
"""
...
def parse_arch(arch, is_html=False):
    """
    Parses a string of XML or HTML into a lxml :class:`etree.ElementTree`.
...
def unparse_arch(tree, doc_header="", is_html=False):
    """
    Converts an etree into a string of XML or HTML.
...
class ArchEditor:
    """
    Context manager to parse an XML or HTML string into an etree, and return it to its original
...
def extract_xpath_keywords(xpath):
    """
    Extracts keywords from an XPath expression. Separates class names from other keywords (e.g. tags, attrs).
...
def extract_xpaths_keywords(xpaths):
    ...
    separating class names from other keywords (eg. attrs, tags).
...
def build_keywords_where_clause(cr, classes, other_kwds, column):
    """
    Builds a WHERE clause to filter records based on keywords.
...
def build_xpaths_where_clause(cr, xpaths, column):
    """
    Builds a WHERE clause to filter records based on XPath expressions.
...
class EtreeConverter:
    ...
    def _compile_conversions(cls, conversions, is_qweb):
        """
        Compiles the given conversions to a list of ``(xpath, operations)`` tuples, with pre-compiled XPaths.
    ...
    def _make_conversions_hashable(cls, conversions):
        """
        Normalizes the given conversions into tuples so they can be hashed.
    ...
    def get_conversions_keywords(self):
        """
        Build and return a list of keywords from the compiled conversions,
        separating class names from other keywords (eg. attrs, tags).
    ...
    def convert_tree(self, tree):
        """
        Converts an etree document inplace with the prepared conversions.
    ...
    def convert_arch(self, arch):
        """
        Converts an XML or HTML arch string with the prepared conversions.
        Returns the converted arch.
    ...
    def convert_file(self, path):
        """
        Converts an XML or HTML file inplace.
...
def convert_views(cr, views_ids, converter):
    ...
    _logger.info(f"Converting {len(views_ids)} views/templates using {converter}")
...
def convert_qweb_views(cr, converter):
    ...
    if not converter.is_qweb:
        raise TypeError(f"Converter for xml views must be ``is_qweb``, got {converter!r}")
...
def convert_html_fields(cr, converter):
    ...
    :rtype: None
    """
    _logger.info(f"Converting html fields data using {converter!r}")
