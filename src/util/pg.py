...
def explode_query_range(cr, query, table, alias=None, bucket_size=DEFAULT_BUCKET_SIZE, prefix=None, explode_on="id"):
    ...
    if min_id is None:
        ...
        if on_CI():
            ...
            parallel_filter = format_query(
                cr,
                "{alias}.{explode_on} IS NOT NULL",
                alias=alias,
    ...
    if count > MAX_BUCKETS:
        ...
        cr.execute(
            format_query(
                ...
                WITH t AS (
                    SELECT {explode_on},
                           mod(dense_rank() OVER(ORDER BY {explode_on}) - 1, %s) AS g
                      FROM {table}
                     WHERE {explode_on} IS NOT NULL
                  ORDER BY {explode_on}
                ) SELECT array_agg(DISTINCT {explode_on} ORDER BY {explode_on}) FILTER (WHERE g=0),
                         min({explode_on}),
                         max({explode_on})
                    FROM t
                """,
    ...
    parallel_filter = format_query(
        cr,
        "{alias}.{explode_on} BETWEEN %(lower-bound)s AND %(upper-bound)s",
        alias=alias,
        explode_on=explode_on,
    )
    query = _explode_format(query.replace("%", "%%"), parallel_filter=parallel_filter)
...
if version_gte("saas~12.3"):
    _COLUMNS[("ir_model_fields", "relation_field_id")] = True  # new in 12
