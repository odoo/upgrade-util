...
if version_gte("15.0"):
    ...
    def profile(func=None, *, description=None, speedscope_file=None):
        ...
        def decorator(cr, *args, **kwargs):
            ...
            if speedscope_file:
                ss = Speedscope(init_stack_trace=profiler.init_stack_trace)
                ss.add("sql", next(c.entries for c in profiler.collectors if c.name == "sql"))
                ss.add("frames", next(c.entries for c in profiler.collectors if c.name == "traces_async"))
                ss.add_output(["frames"], display_name="Frames")
                ss.add_output(["sql"], display_name="SQL")
                ss.add_output(["sql", "frames"], display_name="Combined")

                Path(speedscope_file).write_text(json.dumps(ss.make()))
                _logger.warning("speedscope file saved to %s", speedscope_file)
